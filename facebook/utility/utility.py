...
def get_attacks(scenarios=None, config=None):
    scenarios = scenarios.split(",") if scenarios else range(0, 5)
    config = config.split(",") if config else ["a", "b", "c"]
    return [f"{x}{y}" for x in scenarios for y in config]
