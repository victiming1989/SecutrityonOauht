...
from utility.facebook import is_code_flow, get_facebook_url
...
class Domains(TinyDB):
    ...
    def __init__(self, filename):
        TinyDB.__init__(self, f"{filename}.json", indent=4)
...
class FacebookDomains(Domains):
    ...
    EMPTY_STATE = Query().authorization_url.test(lambda x: get_parameter(get_facebook_url(x), "state") == "")
