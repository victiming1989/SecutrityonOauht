...
def get_google_button(element, keyword="google"):
    ...
    if href and href.startswith("https://accounts.google.com") and not is_google_login(href):
        return None

    keyword = keyword.lower()
    tag = element.name
    # Check if the element contains the keyword in te id
...
def extract_google_domains():
    database = TinyDB("data/_google-01.json", indent=4)
        ...
        if not database.contains(Query().domain == item["site"]):
            ...
            for idp in item["idps"]:
                ...
                if get_google_base_url(idp.get("idp_landing_url", "")):
                    print(item["site"])
                    internal = idp["idp_url"] if get_tld(idp["idp_url"]) != "accounts.google.com" else None
                    domain["idps"] = [{
                        "name": "google.com",
                        "direct": not internal,
                        "internal": internal,
